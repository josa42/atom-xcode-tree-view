...
class TreeView extends ScrollView
  ...
  handleEvents: ->
    ...
    @command 'xcode-tree-view:expand-directory', => @expandDirectory()
    @command 'xcode-tree-view:recursive-expand-directory', => @expandDirectory(true)
    @command 'xcode-tree-view:collapse-directory', => @collapseDirectory()
    @command 'xcode-tree-view:recursive-collapse-directory', => @collapseDirectory(true)
    @command 'xcode-tree-view:open-selected-entry', => @openSelectedEntry(true)
    @command 'xcode-tree-view:move', => @moveSelectedEntry()
    @command 'xcode-tree-view:copy', => @copySelectedEntries()
    @command 'xcode-tree-view:cut', => @cutSelectedEntries()
    @command 'xcode-tree-view:paste', => @pasteEntries()
    @command 'xcode-tree-view:copy-full-path', => @copySelectedEntryPath(false)
    @command 'xcode-tree-view:show-in-file-manager', => @showSelectedEntryInFileManager()
    @command 'xcode-tree-view:open-in-new-window', => @openSelectedEntryInNewWindow()
    @command 'xcode-tree-view:copy-project-path', => @copySelectedEntryPath(true)
    @command 'tool-panel:unfocus', => @unfocus()
    @command 'xcode-tree-view:toggle-vcs-ignored-files', -> atom.config.toggle 'tree-view.hideVcsIgnoredFiles'
    @command 'xcode-tree-view:toggle-ignored-names', -> atom.config.toggle 'tree-view.hideIgnoredNames'

    @on 'xcode-tree-view:directory-modified', =>
      if @hasFocus()
  ...
  detach: ->
    ...
    LocalStorage['xcode-tree-view:cutPath'] = null
    LocalStorage['xcode-tree-view:copyPath'] = null
  ...
  updateRoot: (expandedEntries={}) ->
    ...
    if projectPath = atom.project.getPath()
      ...
      # if @path.match /\.xcodeproj$/
      #   pbpath = path.join @path, 'project.pbxproj'
      #
      #
      #   @path = @path.replace @name, ''
      #
      #   @xcodeProject = xcode.project(pbpath);
      #   @xcodeProject.parseSync()
      #
      #   key = Object.keys(@xcodeProject.hash.project.objects.PBXProject)?[0]
      #   console.log "key: #{key}"
      #
      #   mainKey = @xcodeProject.hash.project.objects.PBXProject[key].mainGroup
      #   mainGroup = @xcodeProject.hash.project.objects.PBXGroup[mainKey]
      #
  ...
  copySelectedEntries: ->
    ...
    LocalStorage.removeItem('xcode-tree-view:cutPath')
    LocalStorage['xcode-tree-view:copyPath'] = JSON.stringify(selectedPaths)
  ...
  cutSelectedEntries: ->
    ...
    return unless selectedPaths and selectedPaths.length > 0
    # save to localStorage so we can paste across multiple open apps
    LocalStorage.removeItem('xcode-tree-view:copyPath')
    LocalStorage['xcode-tree-view:cutPath'] = JSON.stringify(selectedPaths)
  ...
  pasteEntries: ->
    entry = @selectedEntry()
    cutPaths = if LocalStorage['xcode-tree-view:cutPath'] then JSON.parse(LocalStorage['xcode-tree-view:cutPath']) else null
    copiedPaths = if LocalStorage['xcode-tree-view:copyPath'] then JSON.parse(LocalStorage['xcode-tree-view:copyPath']) else null
