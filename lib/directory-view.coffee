...
class DirectoryView extends HTMLElement
  initialize: (@directory) ->
    ...
    if @directory.isRoot
      @directoryName.textContent = "Xcode: #{@directory.name}"
      @directoryName.dataset.name = "Xcode: #{@directory.name}"
    else
  ...
  emitDirectoryModifiedEvent: ->
    if @isExpanded
      event = new CustomEvent('xcode-tree-view:directory-modified', bubbles: true)
      @dispatchEvent(event)
...
DirectoryElement = document.registerElement('xcode-tree-view-directory', prototype: DirectoryView.prototype, extends: 'li')
